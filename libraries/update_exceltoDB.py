...
def duplication_checking_process(filepath,LE):
    ...
    try:
        ...
        # # Load the sheet data into a DataFrame using pandas, skipping the first 7 rows
        # df = pd.DataFrame(sheet.values)
        # df = df.iloc[7:]  # Skip the first 7 rows

        # # Reset the column headers to the 8th row
        # df.columns = df.iloc[0]
        # df = df[1:] 
        # print(df)
        # #new ends
        ...
        df2 = pd.read_excel(filepath, skiprows=7,engine='openpyxl')

        # df2 = df2.iloc[7:]  # Skip the first 7 rows
        ...
        df2 = df2[df2['credit'].isnull()]
        

        debit_count              = df2['debit'].count()  
        duplicate_count          = df2['duplication'].count() 
        voucher_number_Count     = df2['voucher_number'].count()
        
        if debit_count == duplicate_count == voucher_number_Count:
            return "success" 
        else:
            return "failure"
